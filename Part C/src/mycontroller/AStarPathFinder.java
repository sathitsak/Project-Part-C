...
public class AStarPathFinder {
	...
	public AStarPathFinder(World world, int maxSearchDistance, 
		...
		nodes = new Node[world.getWidthInTiles()][world.getHeightInTiles()];
	 ...
	 */
	public Path findPath(Car car, int sx, int sy, int tx, int ty) {
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(car,sx,sy,xp,yp)) {
						...
						float nextStepCost = current.cost + getMovementCost(car, current.x, current.y, xp, yp);
...
//						world.pathFinderVisited(xp, yp);
						...
						if (!inOpenList(neighbour) && !(inClosedList(neighbour))) {
							...
							neighbour.heuristic = getHeuristicCost(car, xp, yp, tx, ty);
	 ...
	 * 
	 * @param car The car that would hold a given location
	 ...
	 */
	protected boolean isValidLocation(Car car, int sx, int sy, int x, int y) {
		boolean invalid = (x < 0) || (y < 0) || (x >= world.getWidthInTiles()) || (y >= world.getHeightInTiles());
	 ...
	 * 
	 * @param car The entity that is being moved
	 ...
	 */
	public float getMovementCost(Car car, int sx, int sy, int tx, int ty) {
...
//		return world.getCost(car, sx, sy, tx, ty);
	 ...
	 */
	public float getHeuristicCost(Car car, int x, int y, int tx, int ty) {
		return heuristic.getCost(world, car, x, y, tx, ty);
	}
